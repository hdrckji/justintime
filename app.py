...
UNASSIGNED_DEPARTMENT_LABEL = "Sans departement"
...
def init_db() -> None:
    ...
    with get_conn() as conn:
        ...
        ensure_department_schema(conn)
        ensure_employee_schema(conn)

        conn.execute("CREATE INDEX IF NOT EXISTS idx_attendance_employee ON attendance_events(employee_id)")
        conn.execute("CREATE INDEX IF NOT EXISTS idx_attendance_timestamp ON attendance_events(timestamp)")
        ...
        if current_count == 0:
            for idx, name in enumerate(SEED_EMPLOYEES, start=1):
                conn.execute(
                    "INSERT INTO employees (name, badge_id) VALUES (?, ?)",
                    (name, f"RFID-{1000 + idx}"),
                )


def ensure_department_schema(conn: sqlite3.Connection) -> None:
    columns = {row["name"] for row in conn.execute("PRAGMA table_info(departments)").fetchall()}
    if "weekly_target_hours" not in columns:
        conn.execute(
            "ALTER TABLE departments ADD COLUMN weekly_target_hours REAL NOT NULL DEFAULT 35"
        )


def ensure_employee_schema(conn: sqlite3.Connection) -> None:
...
def normalize_event(row: dict[str, Any]) -> dict[str, Any]:
    ...
    event["department_name"] = event.get("department_name") or UNASSIGNED_DEPARTMENT_LABEL
    return event
...
def sanitize_weekly_target_hours(value: Any) -> float:
    try:
        hours = float(value)
...
def get_active_employees(conn: sqlite3.Connection) -> list[dict[str, Any]]:
    rows = conn.execute(
        """
        SELECT e.id,
               e.name,
...
def get_latest_status_by_employee(conn: sqlite3.Connection) -> dict[int, str]:
    rows = conn.execute(
        """
        SELECT a.employee_id, a.event_type
        FROM attendance_events a
        INNER JOIN (
            SELECT employee_id, MAX(id) AS max_id
            FROM attendance_events
            GROUP BY employee_id
        ) last_event ON a.id = last_event.max_id
        """
    ).fetchall()
    return {int(row["employee_id"]): str(row["event_type"]) for row in rows}


def get_events_today_count(conn: sqlite3.Connection) -> int:
    row = conn.execute(
        """
...
def get_departments(conn: sqlite3.Connection) -> list[dict[str, Any]]:
    rows = conn.execute(
        """
        SELECT d.id,
               d.name,
...
def get_departments_count(conn: sqlite3.Connection) -> int:
    row = conn.execute("SELECT COUNT(*) AS count FROM departments").fetchone()
    return int(row["count"] if row else 0)


def get_recent_events(conn: sqlite3.Connection) -> list[dict[str, Any]]:
    rows = conn.execute(
        """
        SELECT a.id,
               a.timestamp,
    ...
    return [normalize_event(dict(row)) for row in rows]
...
def get_seconds_since_timestamp(timestamp: str) -> int | None:
    ...
    if event_time is None:
        return None
    return int((datetime.now() - event_time).total_seconds())
...
def get_employee_worked_hours_in_range(
    ...
    for row in rows:
        event_time = parse_timestamp(row["timestamp"])
        if event_time is None:
            continue
        ...
        if row["event_type"] == "out" and open_in is not None:
            start = max(open_in, period_start)
            end = min(event_time, period_end)
            if end > start:
                total_seconds += (end - start).total_seconds()
            open_in = None

    if open_in is not None and period_end > max(open_in, period_start):
        total_seconds += (period_end - max(open_in, period_start)).total_seconds()
...
def build_department_stats(
    ...
    stats_by_department: dict[str, dict[str, Any]] = {}

    for employee in employees:
        department_id = employee.get("department_id")
        key = str(department_id) if department_id is not None else "unassigned"
        department_name = employee.get("department_name") or UNASSIGNED_DEPARTMENT_LABEL

        if key not in stats_by_department:
            stats_by_department[key] = {
                "id": department_id,
                ...
                "absent_count": 0,
                "present_members": [],
                "planned_hours_day": 0.0,
                "planned_hours_week": 0.0,
                "planned_hours_month": 0.0,
        ...
        item = stats_by_department[key]
        item["employees_total"] += 1

        weekly_target = float(employee.get("weekly_target_hours") or DEFAULT_WEEKLY_TARGET_HOURS)
        daily_target = weekly_target / WORK_DAYS_PER_WEEK
        monthly_target = weekly_target * WEEKS_PER_MONTH

        item["planned_hours_day"] += daily_target
        item["planned_hours_week"] += weekly_target
        item["planned_hours_month"] += monthly_target

        employee_id = int(employee["id"])
        item["worked_hours_day"] += get_employee_worked_hours_in_range(conn, employee_id, day_start, now)
        item["worked_hours_week"] += get_employee_worked_hours_in_range(conn, employee_id, week_start, now)
        item["worked_hours_month"] += get_employee_worked_hours_in_range(conn, employee_id, month_start, now)

        if employee.get("status") == "present":
            item["present_count"] += 1
            item["present_members"].append(employee["name"])
        else:
            item["absent_count"] += 1

    stats = list(stats_by_department.values())
    for item in stats:
        total = item["employees_total"]
        item["present_members"] = sorted(item["present_members"])
        item["absenteeism_rate"] = round((item["absent_count"] / total) * 100, 1) if total else 0.0
        for key in (
            "planned_hours_day",
            "planned_hours_week",
            "planned_hours_month",
    ...
    stats.sort(key=lambda item: (item["name"] == UNASSIGNED_DEPARTMENT_LABEL, item["name"].lower()))
    return stats
...
def create_app() -> Flask:
    ...
    def dashboard() -> Any:
        with get_conn() as conn:
            ...
            status_by_employee = get_latest_status_by_employee(conn)

            employee_statuses: list[dict[str, Any]] = []
            present_count = 0
            for employee in employees:
                is_present = status_by_employee.get(int(employee["id"]), "out") == "in"
                if is_present:
                    present_count += 1
            ...
            return jsonify(
                {
                    "summary": {
                        ...
                        "absent": max(len(employee_statuses) - present_count, 0),
                        "events_today": get_events_today_count(conn),
                        "departments_total": get_departments_count(conn),
                    },
    ...
    def create_department() -> Any:
        ...
        weekly_target_hours = sanitize_weekly_target_hours(payload.get("weekly_target_hours"))
        ...
        return jsonify({"message": "Departement ajoute.", "department": dict(row)}), 201
