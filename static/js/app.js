...
function updateCalendarPeriodInputs() {
  if (!els.employeeCalendarPeriod) {
    return;
  }

  const isCustom = els.employeeCalendarPeriod.value === 'custom';
  if (els.employeeCalendarFromWrap) {
    els.employeeCalendarFromWrap.style.display = isCustom ? 'block' : 'none';
  }
  if (els.employeeCalendarToWrap) {
    els.employeeCalendarToWrap.style.display = isCustom ? 'block' : 'none';
  }
}
...
function openEmployeeCalendar(employeeId, employeeName) {
  ...
  if (els.employeeCalendarPeriod) {
    els.employeeCalendarPeriod.value = 'week';
  }
  if (els.employeeCalendarAnchor) {
    els.employeeCalendarAnchor.value = today;
  }
  if (els.employeeCalendarFrom) {
    els.employeeCalendarFrom.value = today;
  }
  if (els.employeeCalendarTo) {
    els.employeeCalendarTo.value = today;
  }
  updateCalendarPeriodInputs();

  if (els.employeeCalendarTitle) {
    els.employeeCalendarTitle.textContent = `Pointages de ${employeeName}`;
  }
  if (els.employeeCalendarSubtitle) {
    els.employeeCalendarSubtitle.textContent = 'Chargement...';
  }
...
function renderEmployeeCalendarSummary(days = [], employee = {}, period = {}) {
  if (!els.employeeCalendarSummary) {
    return;
  }
  ...
  els.employeeCalendarSummary.innerHTML = `
    ...
    <div class="calendar-summary-card">
      <p>Période</p>
      <strong>${period.from_date || ''} → ${period.to_date || ''}</strong>
    </div>
    <div class="calendar-summary-card">
      <p>Heures pointées</p>
    ...
    <div class="calendar-summary-card">
      <p>Jours incomplets</p>
      <strong>${incompleteDays}</strong>
    </div>
  `;
}

function renderEmployeeCalendarGrid(days = [], period = {}) {
  if (!els.employeeCalendarGrid) {
    return;
  }
  ...
  const headers = useWeekdayGrid
    ? weekdays.map((label) => `<div class="calendar-weekday">${label}</div>`).join('')
    : '';
  const spacers = period.type === 'month'
    ? Array.from({ length: firstWeekday }, () => '<div class="calendar-spacer"></div>').join('')
    : '';

  const cards = days.map((day) => {
    const eventsHtml = (day.events || []).length
      ? day.events.map((event) => `
          <span class="calendar-chip ${event.event_type}">
            ${event.time} ${event.event_type === 'in' ? 'Entrée' : 'Sortie'}
          </span>
        `).join('')
    ...
    return `
      <article class="calendar-day ${day.status}">
        <div class="calendar-day-head">
          ...
          <span>${day.event_count || 0} pointage(s)</span>
        </div>
...
function wireEvents() {
  ...
  if (els.employeeCalendarClose) {
    els.employeeCalendarClose.addEventListener('click', closeEmployeeCalendar);
  }

  if (els.employeeCalendarModal) {
    els.employeeCalendarModal.addEventListener('click', (event) => {
      if (event.target === els.employeeCalendarModal) {
        closeEmployeeCalendar();
      }
    });
  }

  if (els.employeeCalendarApply) {
    els.employeeCalendarApply.addEventListener('click', () => {
      loadEmployeeCalendar().catch((error) => showToast(error.message, true));
    });
  }

  if (els.employeeCalendarPeriod) {
    els.employeeCalendarPeriod.addEventListener('change', updateCalendarPeriodInputs);
  }
